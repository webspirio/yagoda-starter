...
src/
  ...
  payouts/                # cash over the counter — `code` server-generated like an intake's, `amount` still sent (§3.7 allows any sum from 0 to «Разом»); the debt half of §3.6's ceiling behind a supplier row lock; `settle-return` records cash physically coming back after a void
