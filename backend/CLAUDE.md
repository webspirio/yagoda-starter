...
src/
  ...
  users/                 # User, UserIdentity, UserCredentials entities; UsersService, CredentialsService; password-hashing.ts (scrypt), secret-box.ts (the owner-readable password vault, AES-256-GCM), normalize-login.ts, display-name.ts (domain only — no controller)
  ...
  shifts/                 # one point's working day — the ONLY home of a document's point and business_date. Open/close are OPERATOR-only (§10.3) and each CARRIES A CASH COUNT in the same transaction; reopen is owner-only and demotes that shift's closing count to `midday`. A discrepancy never blocks a close (client ruling 09.09.2026), so `awaiting_explanation` is unreachable by decision; `explanation` is written by the owner AFTER the fact. Closing ALSO records §6.8's «бій» (`broken_crates`, #110) in that same transaction — required on the DTO because a number that can be skipped gets skipped, `NULL` meaning «не записано» and `0` «нічого не побилось»; reopen clears it back to `NULL`, and the overwritten value survives only in the audit log. `ShiftResponse` carries `opened_by_name`/`closed_by_name` (point-cash parity R3, 2026-09-22) — `users/display-names.ts`'s `loadDisplayNames()`, one `IN (...)` query per response/list rather than one per row, `null` when the id has no match
  intakes/                # the berry receipt — one aggregate, three entities, ONE POST in one transaction. `code` is SERVER-GENERATED (`common/document-code.ts`), not sent: the typed receipt-number field left the DTO and the form on 2026-09-18, which also made `CreateIntakeDto` and `PreviewIntakeDto` the same class. `intake-lines.ts` is pure and holds every computation in the slice. `POST /intakes/preview` (200, both roles) is `create` minus the write — the SAME body (identical class since `code` left it), same snapshots, same refusals — for the reception screen's live numbers. Since 2026-09-21 `POST /intakes` takes an optional `paid_amount` and writes the payout handed over with the receipt IN THE SAME TRANSACTION through `PayoutsService.writePayout` (§2.1 ⑥, §3.1); `GET /intakes` rows carry `net_kg`, `lines_count`, `supplier_name`, `paid_amount` (one SQL definition in `intake-row-extras.ts`, the programme's first shared read), and the detail adds `payouts` and `received_by_name`. `GET /intakes?expand=items` nests each row's lines, `product_name`/`grade_name` joined at read time — so a screen that shows what a receipt contained reuses THIS read instead of a detail request per row, or a second endpoint (#148, and the journal's #102 next). Without the flag the key is ABSENT, never `[]`: a receipt with no lines cannot exist, so «asked, found none» must not be indistinguishable from «never asked».
  payouts/                # cash over the counter — `code` server-generated like an intake's, `amount` still sent (§3.7 allows any sum from 0 to «Разом»); §3.6's ceiling in full (debt AND drawer) behind a supplier row lock; `settle-return` records cash physically coming back after a void. `writePayout(manager, …)` is the ONE writer — supplier lock → open shift → debt → number (the `PO` advisory lock) → cash (`PointCashService.cashFor`, §3.6's second half, read UNDER that lock since 2026-09-21 so two payouts at one point cannot both pass it) → insert; `intake_id` (nullable) says which receipt the cash left with, and is NOT a debt allocation (§3.3 correction).
  ...
  supplier-balance/       # owns ONE query: Σ intakes + Σ intake_top_ups − Σ payouts, with `voided_at IS NULL` on all four (the top-ups term carries two — itself and its parent intake). ONE query, TWO SHAPES. Per supplier (`GET /suppliers/:id/balance`) the response opens the number up: `intakes_total`, `top_ups_total`, `payouts_total` — the three terms `debt` is made of — plus the season counters the card's tiles would otherwise have to sum off a page that truncates, `intakes_count`, `kg_total` and `last_intake_date` (#103); a card that prints only «Разом» cannot say where it came from. Per point (`GET /supplier-balances`, the «Залишки» list) it stays the bare `debt` — same SQL correlated per row, paginated in Postgres, `include_zero=false` by default but a deactivated supplier with a balance stays listed — because a list that renders one column must not pay per row for six aggregates nobody reads. Writes nothing, owns no table
  point-cash/              # owns ONE query: accepted transfers in, payouts out, settled returns back, as of a date. Writes nothing, owns no table, same shape as `supplier-balance/`. Serves `GET /point-cash` (the one screen both roles open, §7.10) and `GET /point-cash/:pointId`. Voided PAYOUTS stay subtracted while voided TRANSFERS stop being added — the same column read two opposite ways, §9.3. EVERY COLUMN OF A LIST ROW HONOURS `as_of`, not just `cash`: `unexplained_difference` is bounded by the count's `business_date` and `latest_transfer` is reconstructed to the status it HELD on that date, because a row pairing a September-1 drawer with today's newest trip reads as fact and is not one. Also serves `crate_deposits` — the crates book (`Σ deposit_taken − Σ deposit_refund`, from `crates/crate-balance.service.ts`'s `crateBookSql`), a SEPARATE field never summed with `cash` and never bounded by `as_of` (§7.5 — «від першої видачі», no date floor at all). `crate_deposit_units` sits beside it (point-cash parity R8, 2026-09-22) — same book, in crates rather than gryvnias: `Σ units` of every live deposit-mode issuance minus `Σ crate_return_allocations.units` returned against one, via `crateUnitsSql` next to `crateBookSql`; an INTEGER (SQL casts `::int`, so the driver hands it back as a JS `number` — never a decimal string, never parsed in TypeScript), also never bounded by `as_of`
  cash-counts/            # the drawer, counted by a human (§7.6). `GET` is read-only, scoped by role; `POST /cash-counts` (operator-only, point-cash parity R2, 2026-09-22) is now a real write route — a MIDDAY RECOUNT, §7.6's witness. It does NOT replace the mandatory counts a shift's open/close still carry in their own transactions (that guarantee — a count that can be written on its own gets skipped — is exactly why open/close write theirs inline), and it does not open an incident: `is_open` stays `false` for every `midday` row and `only_discrepancies` still excludes `kind = 'midday'` unconditionally — the CLOSING count is still what carries the day. `GET /cash-counts?only_discrepancies=true` is the owner's incident list, which is what «notify the owner» means in a project with no email and no push. That list EXCLUDES `midday`, as do `is_open` and `point-cash`'s `unexplained_difference`: a reopen leaves two rows for one physical drift (the demoted closing count and the re-close), and counting both reports −180 where the drawer is −90 out. The demoted row keeps its place in the UNFILTERED list — evidence is never destroyed (§7.6), it just stops being work. `CashCountRowResponse` carries `counted_by_name` (R3, same slice, `users/display-names.ts`). `CashBook` (`cash-book.enum.ts`) now has a real `crates` member, but no row is ever written with it: the crates book is DERIVED ONLY (`point-cash/`), because one physical drawer cannot become two counted numbers without either asking the operator to distinguish identical banknotes or making one book unfalsifiable — see the settlement-trio follow-up
  crates/                 # the rented-crate ledger — `crate_issuances` (deposit or receipt, one `code` either way), `crate_returns`, `crate_return_allocations` (the FIFO trail). Two services because the write and read paths share only the allocator: `crates.service.ts` (issue · return · void × 2, under a `SELECT … FOR UPDATE` on the supplier row, §7.3) and `crate-balance.service.ts` (tranches · balance · preview, and `crateBookSql`/`crateUnitsSql`, the crates book's one definition in money and in units — `point-cash/` reads it rather than re-deriving the filter). FOUR controllers because neither `/suppliers/:id/crate-balance` nor `/shifts/:id/crates` can live on a `/crate-issuances` prefix — the latter is `crate-dispatch.service.ts`, §6.8's dispatch line (#110): `with_berry` (Σ crate tare on the shift's live intakes), `broken` (the shift column) and their sum `dispatched`, which is `null` whenever `broken` is. It reads, stores nothing, and serves an OPEN shift on purpose — the close form renders «з ягодою» while the operator is still typing the breakage. `crate-allocation.ts` is the pure FIFO walk, no Nest, no database, ordered `created_at` then `id`; `mode` is display-only, the money follows `per_unit`, which a receipt-mode issuance carries as 0 by CHECK — so the allocator never branches on mode (§4.1 of the slice spec, which also overrules §6.6's «не змішуються»). `crate-code.ts` is now a thin wrapper over `common/document-code.ts`'s `nextDocumentCode` — all four document kinds are numbered there since 2026-09-18 — and adds the one thing that is still specific to crates: a `mode` scope, so CR and CD count separately inside one table. No `tare_type_id` on either table: one standard crate network-wide, `is_crate` exclusive on `tare_types` (`UQ_tare_types_single_crate`, demotes-on-set in `tare-types.service.ts`). No `PATCH`, no `DELETE`, no `crate_shipments` — ledger only, see the slice spec's §9 and the follow-ups doc
  user-admin/             # owner-only POST /users, PATCH /users/:id, PUT /users/:id/password, GET /users/:id/password (reads a password back, issue #11 — audited, and the ONLY route that ever returns one) — the only way an account is created
  ...
  migrations/             # InitialSchema, SeedDevAdmin (guarded off in production), YagodaFoundation, BootstrapOwner, IndexUserIdentityUser, YagodaCatalog, YagodaSuppliersAndPrices, YagodaIntakesAndPayouts, YagodaTransfers, YagodaCashCounts, DropCashCountExpectedCheck, YagodaIntakeTopUps, YagodaCrates, UserPasswordVault, YagodaReweigh, YagodaDayExpenses, ShiftBrokenCrates, PayoutIntakeLink
...
- **The owner can read a password back, and that is a second, reversible copy — never the login path.** Issue #11 («бачити логін та пароль кожного користувача») cannot be served by a hash, so `CredentialsService.set()` also writes `user_credentials.password_enc`: the same password sealed with AES-256-GCM by `src/users/secret-box.ts` under `PASSWORD_VAULT_KEY` (32 bytes, base64, validated at boot, **never in the database**). `CredentialsService.reveal()` is the only reader and `GET /users/:id/password` (owner-only, audited as `user.password-viewed`) the only route. Four things hold this in place and must keep holding: `verify()` still checks the scrypt hash and never consults the copy, so deleting every `password_enc` locks nobody out; an unset key is a supported deployment in which no copy is written and none is read; `set()` writes the column on EVERY call, as `null` when there is no key, so a stale copy can't outlive a reissue; and `null` on the read means «reissue the password», never «no password». Never wire login to this column, and never let a list or detail response carry a password.
...
- **ALL money and weight arithmetic goes through `src/common/money.ts`, and nothing else may do any.** Strings in, strings out, bigint kopiykas inside, half-up at scale 2. **Rounding is applied PER LINE and the rounded values are then summed** — the receipt prints the lines above the total, and `round(Σ)` can differ from `Σ round(each)` by a kopiyka, which the schema treats as no different from 350 ₴. An eslint rule (`eslint.config.mjs`) bans `*`, `/`, `*=`, `/=`, `Number()`, `parseInt`, `parseFloat` and `toFixed` in the modules its money `files` array names — read the list there, never from a copy: every module owning a money or weight `numeric` column must be in it. Pagination offsets use `skipOf()` from `common/dto/pagination-query.dto.ts` rather than a disable comment. `decimal.js` is deliberately still not a RUNTIME dependency — when the arithmetic stops being provisional it replaces the internals of that one file. It IS a devDependency, used only as the independent oracle in `money.properties.spec.ts`; that is the opposite of reversing the deferral, since it is how the bigint internals get checked before anything depends on swapping them.
- **Every document code is generated by the server, in one place.** `common/document-code.ts` composes `{POINT}-{IN|PO|CR|CD}-{YYYYMMDD}-{NNN}` and `nextDocumentCode` allocates the sequence inside the write transaction — `pg_advisory_xact_lock` keyed on (table, shift, kind), then a row count that INCLUDES voided rows, so a number is never reissued. The counter key is the SHIFT, because `UQ_shifts_point_business_date` makes a shift exactly one (point, day); no counters table exists or is needed. `IN` and `PO` joined `CR`/`CD` here on 2026-09-18, when the client removed the typed receipt-number field from both forms — the operator now copies the system's number ONTO the paper book rather than reading one off it. Neither `CreateIntakeDto` nor `CreatePayoutDto` accepts a `code`, and `forbidNonWhitelisted` means a client that still sends one gets a 400.
- **A document is never edited.** §2.7 freezes `intakes.amount` and §9.3 makes a correction a void plus a NEW document, so there is no `PATCH` and no update method on `IntakesService`/`PayoutsService` — only a `void` that writes the mandatory `void_*` trio. An operator may void only a document they RECORDED THEMSELVES, and only while the shift is open (§9.4 — «чужа квитанція → приймальник НІКОЛИ»); the owner may void anything, anywhere. Voiding a payout does NOT return the cash: that is a separate owner-only `settle-return` (§9.3, «інакше сторно стає способом красти»). A payout made at reception carries `intake_id`; voiding the intake does not void it (§3.5's recorded exception) and voiding the payout does not touch the intake.
...
**RE-SEEDING A DATABASE SEEDED BEFORE 2026-09-18 DOUBLES ITS DOCUMENTS.** The seed is
idempotent by `code`, and on that date `code` changed shape: intakes and payouts are now
numbered `…-IN-<date>-001` per shift like every other document, where they used to carry the
paper-book number from `typed`. Every lookup therefore misses, and every receipt and payout is
inserted a second time. There is no repair step and none is planned — this is a dev fixture:
drop the database (or `docker compose down -v`) and seed again. `app_test` is unaffected,
since `openTestDataSource()` recreates it on every call.
