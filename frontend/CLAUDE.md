...
src/
  ...
  features/count-shift/          # useOpenShiftMutation / useCloseShiftMutation, CountDrawerDialog, OpenShiftAlert — opens or closes a shift against the counted drawer (§10.3), used by pages/day AND pages/reception — the reception consumer is the whole reason this lives here rather than inside pages/day. `OpenShiftAlert` (#114) is the second reason: every screen reads the shift of the day it is ABOUT, so a shift left open five days ago is invisible on both; the alert reads `GET /shifts/current` (date-blind), warns, and closes it IN PLACE — it composes entities/shift's read with this slice's own mutation and dialog, which is why it sits here and not in either page
