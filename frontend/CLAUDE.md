...
src/
  ...
  entities/shift/                # useShiftOnDateQuery / useCurrentShiftQuery / shiftOnDateQueryOptions / useStaleOpenShiftsQuery, Shift type — one point's working day, read by pages/day and reception (the queryOptions factory also backs pages/dashboard's useNetworkToday fan-out). `useStaleOpenShiftsQuery` (#114) is the odd one out: it asks `GET /shifts?status=open&to=<вчора>` with NO point, which for an owner is the whole network in one request — the owner's «Незакриті зміни» on the dashboard
  ...
  features/count-shift/          # useOpenShiftMutation / useCloseShiftMutation, CountDrawerDialog / RecountDrawerDialog, CountResultView + discrepancyTone, OpenShiftAlert — opens, closes (§10.3) and mid-shift recounts (§7.6, `POST /cash-counts`, R2/D-6) a shift against the counted drawer, used by pages/day, pages/reception AND pages/point-cash — RecountDrawerDialog owns its own `useRecountMutation` (api/recount.ts) by relative import, not barrel-exported (no caller outside this feature needs it); CountResultView IS shared through the barrel — both CountDrawerDialog's own callers and RecountDrawerDialog render it for a counted result, so a pages/* import reaching down into this feature (the allowed FSD direction) is how the two share it instead of duplicating the shape. `OpenShiftAlert` (#114) is the other reason it is a feature: every screen reads the shift of the day it is ABOUT, so a shift left open five days ago is invisible on all of them; the alert reads `GET /shifts/current` (date-blind), warns, and closes it IN PLACE — it composes entities/shift's read with this slice's own mutation and dialog, which is why it sits here and not in any one page
