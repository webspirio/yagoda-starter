...
src/
  ...
  features/count-shift/          # useOpenShiftMutation / useCloseShiftMutation / useRecountMutation, CountDrawerDialog / RecountDrawerDialog, CountResultView + discrepancyTone — opens, closes (§10.3) and mid-shift recounts (§7.6, `POST /cash-counts`, R2/D-6) a shift against the counted drawer, used by pages/day, pages/reception AND pages/point-cash — CountResultView is the one component both CountDrawerDialog's own callers and RecountDrawerDialog render for a counted result, so a pages/* import reaching down into this feature (the allowed FSD direction) is how the two share it instead of duplicating the shape
