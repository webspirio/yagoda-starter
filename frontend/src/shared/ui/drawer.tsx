...
/**
 * How long vaul's open/close transition runs — its internal
 * `TRANSITIONS.DURATION` (0.5s), which the library does not export.
 *
 ...
 * controlled `open` prop vaul only fires it for closes it initiates itself
 * (swipe, Escape, outside press, `DrawerClose`), never for an `open` flip from
 * the outside. Keeping the number here keeps that knowledge next to the `vaul`
 * import instead of guessed at in a page.
 */
 ...
 * detail panel). Keeps `vaul` confined to `shared/ui` so no page layer
 * imports it directly. `aria-describedby={undefined}` opts out of Radix's
