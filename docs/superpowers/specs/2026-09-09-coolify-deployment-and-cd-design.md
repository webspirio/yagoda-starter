...
- `mem_limit` per service: backend 384M, postgres 256M, redis 64M, nginx 64M, seed 256M — verified under load per §2 before the numbers are committed. `backend` and `seed` set `NODE_OPTIONS=--max-old-space-size=256`, so the heap limit is below the cgroup limit and an OOM is a readable Node error, not a SIGKILL.
...
- **`deploy-preview`** — `needs: [checks, db-checks, docker]`, internal PRs only, `concurrency: preview-allocation` (`cancel-in-progress: false`) so allocation is serialized across the whole repository. Steps:
  ...
  3. Readiness against the application, bounded retry: `GET https://pr-N.yagoda.webspirio.com/api/health/ready` → 200; `GET /api/health/version` → `commit == github.sha`; `POST /api/auth/login` with the seed operator credentials from `dev-seed.data.ts` → 200 (proves the seed completed; those credentials are demo data and never reach production because the seed refuses `NODE_ENV=production`).
