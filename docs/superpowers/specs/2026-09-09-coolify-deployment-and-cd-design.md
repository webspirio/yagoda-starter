...
- **`deploy-preview`** — `needs: [checks, db-checks, docker]`, internal PRs only, `concurrency: preview-allocation` (`cancel-in-progress: false`) so allocation is serialized across the whole repository. GitHub keeps one pending run per concurrency group, so a third simultaneous preview is cancelled rather than queued — re-run it (runbook). Steps:
